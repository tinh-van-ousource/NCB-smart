...
public class CompanyServiceImpl implements CompanyService {
	...
	public CompanyServiceImpl(CompanyRepo companyRepo, EntityManagerFactory entityManagerFactory,
	...
	public CompanyResDto update(CompanyUpdateReqDto companyUpdateReqDto) {
		...
		if (companyEntity == null) {
			...
			if (companyEntity1 == null) {
				...
				if (companyEntity2 == null) {
					companyEntity.setCompName(companyUpdateReqDto.getCompName());
					companyEntity.setDao(companyUpdateReqDto.getDao());
					return ModelMapperUtils.map(companyRepo.save(companyEntity), CompanyResDto.class);
	...
	private Object[] createRootPersist(CriteriaBuilder cb, CriteriaQuery<?> query, CompanySearchReqDto resource) {
		...
		if (StringUtils.isNotEmpty(resource.getCompCode().trim())) {
			predicates.add(cb.and(cb.like(cb.upper(rootPersist.<String>get("compCode")), resource.getCompCode())));
		}

		if (StringUtils.isNotEmpty(resource.getCompName().trim())) {
			predicates.add(cb.and(
					cb.like(cb.upper(rootPersist.get("compName")), "%" + resource.getCompName().toUpperCase() + "%")));
		}
